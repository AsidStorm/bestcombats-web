...
} elseif ($user["room"]!=45 && $user["room"]!=49 && $user["room"]!=54 && $user["room"]!=70 && $user["room"]!=456 && $user["room"]!=9000 && $user["room"]!=26) echo "<script>document.location.replace('main.php".($warning?"?warning=$warning":"")."');</script>";
?>
<HTML><HEAD>
...
if ((int)date("H") > 5 && (int)date("H") < 22) {
    ...
    define("TIME_OF_DAY", 'n');
}

if ($user['room']==20) {
    $hinttop=18;
    $hintright=113;
    $fon = 'sn_bg1_1' . TIME_OF_DAY . SEASON;
    $bk = 'sn_club' . TIME_OF_DAY . SEASON;
    $poct = 'sn_post' . TIME_OF_DAY . SEASON;
    $vok = 'sn_station' . TIME_OF_DAY . SEASON;
    $xram = 'sn_temple' . TIME_OF_DAY . SEASON;
    $repair = 'sn_repair' . TIME_OF_DAY . SEASON;
    $sh = 'sn_comshop' . TIME_OF_DAY . SEASON;
    $opt = 'sn_euroshop' . TIME_OF_DAY . SEASON;
    ...
    buildsetnew(0,"sun/$xram",127,369,"Храм");
    buildsetnew(0,"sun/$repair",156,349, "Ремонтный цех");
    buildsetnew(0,"sun/$sh",154,272, "Закрома старьевщика");
    buildsetnew(0,"sun/$opt",150,190, "Оптовый магазин");
    buildsetnew(0,"sun/$shop",172,53, "Прилавки с товарами");
...
?>
<div id="snow"></div>
<div style="position:absolute; left:407px; top:0px; width:1px; height:1px; z-index:101; overflow:visible;">
...
<? } else { ?>
  <span style="color:#A020F0"  onclick="window.open('/forum.php', 'forum', 'location=yes,menubar=yes,status=yes,resizable=yes,toolbar=yes,scrollbars=yes,scrollbars=yes')">Форум</span> &nbsp;
  <? if($user['room'] == 20){?>
  <span style="color:#A020F0" onclick="findlogin('Введите имя персонажа', 'city.php?nap=attack', 'target'); ">Напасть</span> &nbsp;
  <?}?>
...
-->
    ...
    <BR><br>
        ...
        <?php
        $onlined   = mysql_num_rows(mysql_query("select real_time from `online`  WHERE `city`='dungeon' AND `real_time` >= ".(time()-60).";"));
        ...
        $onlinesun = mysql_num_rows(mysql_query("select real_time from `online`  WHERE `city`='suncity' AND `real_time` >= ".(time()-60).";"));
        ?>
