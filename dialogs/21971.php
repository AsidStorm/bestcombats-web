...
if ($step == 1) {
    db_query("REPLACE INTO bezdna_exam_users SET id = $user[id], questionID = " . mt_rand(1, 10));
...
if ($step == 6) {
    ...
    $ques = mysqli_fetch_assoc(db_query("SELECT * FROM bezdna_exam_questions WHERE volume = $exam[step] AND id = $exam[questionID] LIMIT 1"));
    if (isset($_GET['a'])) {
        if (md5($ques['id']) == $_GET['a']) {
            ...
            db_query("REPLACE INTO bezdna_exam_users SET id = $user[id], step = " . $exam['step'] . ", questionID = " . mt_rand((($exam['step']*10)-9), ($exam['step']*10)));
    ...
    $isPage = db_result(db_query("SELECT COUNT(*) FROM inventory WHERE owner = $user[id] AND prototype = $ques[pageID] AND koll > 0 LIMIT 1"), 0, 0);
    $isBook = db_result(db_query("SELECT COUNT(*) FROM inventory WHERE owner = $user[id] AND prototype = $ques[bookID] AND koll > 0 LIMIT 1"), 0, 0);
    ...
    if ($isBook) {
        ...
        $answers[6 . '&a=' . md5($ques['id'])] = 'У меня есть ' . db_result(db_query("SELECT name FROM shop WHERE id = $ques[bookID] LIMIT 1"), 0, 0) . ' (показать книгу).';
    } elseif ($isPage) {
