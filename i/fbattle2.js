...
function DrawTrick(can_use, img,  txt, free_cast, dsc, resource, select_target, target, target_login, magic_type, name){
  ...
  if (can_use){
    if (select_target){
        s += "<A style='cursor: pointer' onclick=\"tricklogin('</b>Выберите" +
...
function DrawButtons(script_alert){
  document.getElementById('buttons').innerHTML='<input type=checkbox name=autohit onmouseout=\'hideshow()\' onmouseover=\'fastshow2("Автоматический выбор зон блока и удара",this, event)\' value=1 onclick="setautohit(this.checked);"'+(top.autohit?' checked':'')+'>'+
  '&nbsp;&nbsp;&nbsp;<input type=checkbox name=autogo value=1 onmouseout=\'hideshow()\' onmouseover=\'fastshow2("Удар при выставлении хода",this, event)\' onclick="setAutoGo(this.checked);"'+(top.autogo?' checked':'')+'>' +
  '&nbsp;<INPUT TYPE=submit id="let_attack"' + (top.autogo ? ' disabled':'')+ ' value="Вперед !!!" onclick="this.disabled = true; checksubmit(); return false;">' +
...
function note(title, script, name,coma,errk){
  ...
  errkom+'</font>Введите текст</TD></TR><TR><TD width=50% align=right><INPUT TYPE=text NAME="'+name+'" value="'+com+'"></TD><TD width=50%><INPUT TYPE="submit" value=" »» "></TD></TR></TABLE></FORM></td></tr></table>';
