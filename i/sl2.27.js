...
function usescript(title, script, name, def, pretext, mtype, addon) {
	...
	pretext + '</TD></TR><TR><TD width=50% align=right style="padding-left:5"><INPUT style="width: 100%" TYPE="text" NAME="'+name+'" id="'+name+'" value="'+def+'"></TD><TD width=50%><INPUT type=image SRC="#IMGSRC#" WIDTH="27" HEIGHT="20" BORDER=0 ALT="" onclick="slform.'+name+'.value=fixspaces(slform.'+name+'.value);">'+(addon?addon:'')+'</TD></TR></TABLE></FORM>';
...
//		errmess("Aeie ia aua?ai."); return false;
	...
	'Oea?eoa eiaei ia?niia?a:<small><BR>(ii?ii uaeeioou ii eiaeio a ?aoa)</TD></TR><TR><TD width=50% align=right style="padding-left:5"><INPUT style="width: 100%" TYPE="text" NAME="'+name+'" value="'+defaultlogin+'"></TD><TD width=50%><INPUT type=image SRC="#IMGSRC#" WIDTH="27" HEIGHT="20" BORDER=0 ALT="" onclick="slform.'+name+'.value=fixspaces(slform.'+name+'.value);">'+(addon?addon:'')+'</TD></TR></FORM></TABLE>';
	s = crtmagic(mtype, title, s);
...
//		errmess("Aeie ia aua?ai.");	return false;
	...
	dialogconfirm('Iiaoaa??aaiea', '/battle.pl', '<TABLE width=100%><TD><B>'+txt+'</B><BR>Eniieuciaaou nae?an?</TABLE>'+addon, mtype);
}
...
function dialogconfirm(title, script, text, mtype) {
	...
	s='<table border=0 width=100% cellspacing="0" cellpadding="2"><tr><form action="'+script+'" method=POST name=slform><td colspan=2>'+
	text+'</TD></TR><TR><TD width=50% align=left><INPUT TYPE="submit" name="tmpname423" value="Aa" style="width:70%"></TD><TD width=50% align=right><INPUT type=button style="width:70%" value="Iao" onclick="closehint3();"></TD></TR></FORM></TABLE>';
...
function dialogOK(title, text, mtype) {
	...
	text+'</TD></TR><TR><TD width=100% align=right><INPUT type=button style="width:70%" value="Cae?uou" onclick="closehint3();"></TD></TR></FORM></TABLE>';
...
function UseMagick(title, script, name, extparam, n, extparam2, mtype) {
   if ((extparam != null)&&(extparam != '')) {
	...
	Hint3Name = 'param';
		...
		dialogconfirm('Iiaoaa??aaiea', script, '<TABLE width=100%><TD><IMG src="http://img.combats.com/i/items/'+name+'.gif"></TD><TD>Eniieuciaaou nae?an?</TABLE>'+
...
//		errmess("Aeie ia aua?ai.");	return false;
	...
	'Oea?eoa eiaei ia?niia?a:<small><BR>(ii?ii uaeeioou ii eiaeio a ?aoa)</TD></TR><TR><TD width=50% align=right><INPUT style="width: 100%" TYPE="text" id="param" NAME="param" value="'+defaultlogin+'"></TD><TD width=50%><INPUT type=image SRC="#IMGSRC#" WIDTH="27" HEIGHT="20" BORDER=0 ALT="" onclick="bmagic.param.value=fixspaces(bmagic.param.value);"></TD></TR>';
	if (extparam != null && extparam != '') {
...
//		errmess("Aeie ia aua?ai.");	return false;
	...
	if ((extparam != null)&&(extparam != '')) {
		...
		dialogconfirm('Iiaoaa??aaiea', '/battle.pl', '<TABLE width=100%><TD><IMG src="http://img.combats.com/i/items/'+name+'.gif"></TD><TD>Eniieuciaaou nae?an?</TABLE>'+
...
var s='<table width="270" border="0" align="center" cellpadding="0" cellspacing="0">'+
	...
	'<tr><td>'+
		...
		'<tr>'+
			...
			'</td><td width=20 align=right valign=top style="cursor: hand" onclick="closehint3();" style=\'filter:Gray()\' onmouseover="this.filters.Gray.Enabled=false" onmouseout="this.filters.Gray.Enabled=true"><IMG src="http://img.combats.com/i/clear.gif" width=13 height=13>&nbsp;</td></table>'+
			'</td></tr>'+
			'<tr>'+
