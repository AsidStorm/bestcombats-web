<?php
    ...
    if ($user['room'] != 227) { header("Location: main.php");  die(); }
    if ($user['battle'] != 0) { header('location: fbattle.php'); die(); }

    if ($_GET['sed']) {

        $dress = mysqli_fetch_array(db_query("SELECT * FROM `inventory` WHERE `dressed`= 0 AND `id` = '{$_GET['sed']}' AND `owner` = '{$_SESSION['uid']}'  AND `podzem`=0 AND `artefact` = 0 LIMIT 1;"));
        ...
        $ec=mqfa1("select ecost from shopvip where name='$name'");
        ...
        if($dress["podzem"] == 0 && $ec=$dress["ecost"]){
          ...
          //db_query("UPDATE `users` set `ekr` = `ekr`+ '".(round($price-$dress['duration']*($dress['ecost']/($dress['maxdur']*10)),2))."' WHERE id = {$_SESSION['uid']}");
          //$allcost=$dress['ecost']/2;
          db_query("UPDATE `users` set `ekr` = `ekr`+ '".$price."' WHERE id = {$_SESSION['uid']}");
          db_query("INSERT INTO `delo` (`id` , `author` ,`pers`, `text`, `type`, `date`) VALUES ('','0','{$_SESSION['uid']}','\"".$user['login']."\" продал в магазин товар: \"".$dress['name']."\" id:(cap".$dress['id'].") [".$dress['duration']."/".$dress['maxdur']."] за ".$price." екр. ',1,'".time()."');");
    ...
    if (($_GET['set'] OR $_POST['set'])) {
        ...
        $dress = mysqli_fetch_array(db_query("SELECT * FROM `shopvip` WHERE `id` = '{$set}' LIMIT 1;"));
        ...
        elseif(($user['ekr']>= ($dress['ecost']*$_POST['count'])) && ($dress['count'] >= $_POST['count'])) {
            ...
            if ($good) {
                db_query("UPDATE `shopvip` SET `count`=`count`-{$_POST['count']} WHERE `id` = '{$set}' LIMIT 1;");
                echo "<font color=red><b>Вы купили {$_POST['count']} шт. \"".stripslashes($dress['name'])."\".</b></font>";
                db_query("UPDATE `users` set `ekr` = `ekr`- '".($_POST['count']*$dress['ecost'])."' WHERE id = {$_SESSION['uid']} ;");
                ...
                db_query("INSERT INTO `delo` (`id` , `author` ,`pers`, `text`, `type`, `date`) VALUES ('','0','{$_SESSION['uid']}','\"".$user['login']."\" купил товар: \"".$dress['name']."\" ".$dresscount."id:(".$dressid.") [0/".$dress['maxdur']."] за ".$allcost." екр. ',1,'".time()."');");
            }
        }
        else {
...
<tr><td><h3>VIP Магазин</td><td align=right>
...
</table><small>Скидки на все товары 30% только для обладателей VIP статуса!</small>
<TABLE border=0 width=100% cellspacing="0" cellpadding="4">
<TR>
    <FORM METHOD=POST ACTION="shopvip.php">
...
if($_REQUEST['present']) {
    ...
    if($_POST['to_login'] && $_POST['flower']) {
        $to = mysqli_fetch_array(db_query("SELECT * FROM `users` WHERE `login` = '{$_POST['to_login']}' LIMIT 1;"));
        ...
        elseif ($to['room'] > 500 && $to['room'] < 561) {
            ...
            if ($to) if(db_query("UPDATE `inventory` SET `owner` = '".$to['id']."', `present` = '".$from."', `letter` = '".$_POST['podarok2']."' WHERE  `present` = '' AND `id` = '".$_POST['flower']."' AND `owner` = '{$_SESSION['uid']}' AND `dressed` = 0  AND `setsale`=0")) {
...
//print_r($_POST);
    ...
    $data = db_query("SELECT * FROM `inventory` WHERE `owner` = '{$_SESSION['uid']}' AND `dressed` = 0 AND `gift`=1 AND `setsale`=0 AND `present` = '' ORDER by `id` DESC; ");
...
if ($_REQUEST['sale']) {
    $data = db_query("SELECT * FROM `inventory` WHERE `owner` = '{$_SESSION['uid']}' AND `honor` = 0 AND `artefact` = 0 AND `dressed` = 0  AND `setsale`=0  AND `podzem`=0 AND `gift`=0 AND `honor`=0 AND type<>199 and `ecost`>0 ORDER by `update` DESC; ");
    while($row = mysqli_fetch_array($data)) {
        $row['count'] = 1;
        if ($i==0) { $i = 1; $color = '#C7C7C7';} else { $i = 0; $color = '#D5D5D5'; }
        echo "<TR bgcolor={$color}><TD align=center style='width:150px'><IMG SRC=\"".IMGBASE."/i/sh/{$row['img']}\" BORDER=0>";
        ...
        ?>
        <!--<BR><A HREF="shopvip.php?sed=<?=$row['id']?>&sid=&sale=1">продать за <?=round($price-$row['duration']*($row['ecost']/($row['maxdur']*10)),2)?></A>-->
            <BR><A HREF="shopvip.php?sed=<?=$row['id']?>&sid=&sale=1">продать за <?=floor($row['ecost']*SELLCOEF*(($row["maxdur"]-$row["duration"])/$row["maxdur"])*10)/10?> екр</A>
...
{
    $data = db_query("SELECT * FROM `shopvip` WHERE `count` > 0 AND `razdel` = '{$_GET['otdel']}' ORDER by `nlevel` ASC, `ecost` ASC");
    while($row = mysqli_fetch_array($data)) {
        if ($i==0) { $i = 1; $color = '#C7C7C7';} else { $i = 0; $color = '#D5D5D5'; }
        echo "<TR bgcolor={$color}><TD align=center style='width:150px'><IMG SRC=\"".IMGBASE."/i/sh/{$row['img']}\" BORDER=0>";
        ?>
        <BR><A HREF="shopvip.php?otdel=<?=$_GET['otdel']?>&set=<?=$row['id']?>&sid=">купить</A>
    ...
    $user8 = mysqli_fetch_array(db_query("SELECT ekr FROM `users` WHERE `id` = '{$_SESSION['uid']}' LIMIT 1;"));
?>
</TABLE>
</TD></TR>
</TABLE>
<?
    if ($_GET['otdel']==72) {
      echo "Вы можете приобрести любые вещи из ассортимента Берёзки с этими или другими изображениями и названием по своему выбору. Т. е. вы покупаете любую вещь и устанавливаете на неё любую из этих картинок, и у неё меняется название по вашему желанию. При этом можно улучшить модификаторы вещи, но не более чем на 10 каждый и не более чем на 30 в сумме.
...
<?
  function shoplink($otdel, $name, $caption=0) {
    return "<A HREF=\"shopvip.php?otdel=$otdel\">".($otdel==$_GET["otdel"]?"<DIV style='background-color: #C7C7C7'>":"").($caption?"":"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;").$name.($otdel==$_GET["otdel"]?"</div></a>":"</a><br>")."";
  }
