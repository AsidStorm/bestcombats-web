<?php
    ...
    if ($_GET['sed']) {
        ...
        $dress = mysql_fetch_array(mysql_query("SELECT * FROM `inventory` WHERE `dressed`= 0 AND `id` = '{$_GET['sed']}' AND `owner` = '{$_SESSION['uid']}'  AND `podzem`=0 AND `artefact` = 0 LIMIT 1;"));
    ...
    if (($_GET['set'] OR $_POST['set'])) {
        ...
        $dress = mysql_fetch_array(mysql_query("SELECT * FROM `shopvip` WHERE `id` = '{$set}' LIMIT 1;"));
        ...
        elseif(($user['ekr']>= ($dress['ecost']*$_POST['count'])) && ($dress['count'] >= $_POST['count'])) {
            ...
            if ($good) {
                mysql_query("UPDATE `shopvip` SET `count`=`count`-{$_POST['count']} WHERE `id` = '{$set}' LIMIT 1;");
...
if ($_REQUEST['sale']) {
    $data = mysql_query("SELECT * FROM `inventory` WHERE `owner` = '{$_SESSION['uid']}' AND `honor` = 0 AND `artefact` = 0 AND `dressed` = 0  AND `setsale`=0  AND `podzem`=0 AND `gift`=0 AND `honor`=0 AND type<>199 and `ecost`>0 ORDER by `update` DESC; ");
    while($row = mysql_fetch_array($data)) {
        $row['count'] = 1;
        if ($i==0) { $i = 1; $color = '#C7C7C7';} else { $i = 0; $color = '#D5D5D5'; }
        echo "<TR bgcolor={$color}><TD align=center style='width:150px'><IMG SRC=\"".IMGBASE."/i/sh/{$row['img']}\" BORDER=0>";
...
{
    $data = mysql_query("SELECT * FROM `shopvip` WHERE `count` > 0 AND `razdel` = '{$_GET['otdel']}' ORDER by `nlevel` ASC, `ecost` ASC");
    while($row = mysql_fetch_array($data)) {
        if ($i==0) { $i = 1; $color = '#C7C7C7';} else { $i = 0; $color = '#D5D5D5'; }
        echo "<TR bgcolor={$color}><TD align=center style='width:150px'><IMG SRC=\"".IMGBASE."/i/sh/{$row['img']}\" BORDER=0>";
        ?>
