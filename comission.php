<?php
    ...
    $d = mysqli_fetch_array(db_query("SELECT sum(`massa`) FROM `inventory` WHERE `owner` = '{$_SESSION['uid']}' AND `dressed` = 0 AND `setsale` = 0 and clan=''; "));
    ...
    if ($_GET['sale'] && $_GET['kredit'] && $_GET['n']) {
        ...
        if ((is_numeric($_GET['kredit']) && $_GET['kredit']>=1) && (is_numeric($_GET['n']) &&  $_GET['n']>0)) {
            $dress = mysqli_fetch_array(db_query("SELECT * FROM `inventory` WHERE  `dressed`=0 AND podzem<>1 and podzem<>2 AND `id` = '{$_GET['n']}' AND `owner` = '{$_SESSION['uid']}' $cond LIMIT 1;"));
            if (!mqfa1('SELECT COUNT(*) FROM clanstorage WHERE id_it = ' . $dress['id'])) {
                ...
                if ($_GET['kredit']<$prc["minprice"]*1.1) {
                  ...
                  echo "<font color=red><b>Максимальная цена продажи ".($prc["maxprice"]*50)." кр</b></font>";              
                    ...
                    db_query("UPDATE `inventory` SET `setsale` = '".$_GET['kredit']."' WHERE `id` = '{$_GET['n']}' AND `owner` = '{$_SESSION['uid']}' LIMIT 1;");
                    db_query("INSERT INTO `delo` (`id` , `author` ,`pers`, `text`, `type`, `date`) VALUES ('','0','{$_SESSION['uid']}','\"".$user['login']."\" сдал предмет: \"".$dress['name']."\" ".($dress["koll"]?"($dress[koll] шт.)":"")." id:(cap".$dress['id'].") [".$dress['duration']."/".$dress['maxdur']."] в комиссионку за ".$_GET['kredit']." кр. ',1,'".time()."');");
    ...
    if ($_GET['back']) {
        if ($user['money'] >= 1) {
            if (is_numeric($_GET['back']) && $_GET['back']>0) {
                $dress = mysqli_fetch_array(db_query("SELECT * FROM `inventory` WHERE `dressed`=0 AND  `id` = '{$_GET['back']}' AND `owner` = '{$_SESSION['uid']}' AND `setsale` > '0' LIMIT 1;"));
                if($dress['id']) {
                    db_query("UPDATE `users` set `money` = `money`-'1' WHERE id = {$_SESSION['uid']}");
                    db_query("UPDATE `inventory` SET `setsale` = '0' WHERE `id` = '{$_GET['back']}' AND `owner` = '{$_SESSION['uid']}' LIMIT 1;");
                    db_query("INSERT INTO `delo` (`id` , `author` ,`pers`, `text`, `type`, `date`) VALUES ('','0','{$_SESSION['uid']}','\"".$user['login']."\" забрал предмет: \"".$dress['name']."\" ".($dress["koll"]?"($dress[koll] шт.)":"")." id:(cap".$dress['id'].") [".$dress['duration']."/".$dress['maxdur']."] из комиссионки ',1 кр ($user[money]/$user[ekr]),'".time()."');");
                    db_query("INSERT INTO `delo` (`id` , `author` ,`pers`, `text`, `type`, `date`) VALUES ('','0','{$_SESSION['uid']}','\"".$user['login']."\" заплатил 1 кр за хранение предмета: \"".$dress['name']."\" ".($dress["koll"]?"($dress[koll] шт.)":"")." id:(cap".$dress['id'].") [".$dress['duration']."/".$dress['maxdur']."] в комиссионке ',1,'".time()."');");
    ...
    if (($_GET['set'] OR $_POST['set'])) {
        ...
        if (is_numeric($set) && $set>0) {
            $dress = mysqli_fetch_array(db_query("SELECT * FROM `inventory` WHERE `dressed`=0 AND `id` = '{$set}' AND `setsale` > '0' LIMIT 1;"));
            checkuserbyid($dress['owner']);
            $userfrom = mysqli_fetch_array(db_query("SELECT `login`,`id` FROM `users` WHERE  `id` = '".$dress['owner']."' LIMIT 1;"));
            if (($userfrom['id'] || !$dress['owner']) && $dress['id']) {
                ...
                elseif ($user['money'] >= $dress['setsale']) {
                    if(db_query("UPDATE `inventory` SET `owner` = '{$user['id']}', `setsale` = 0 WHERE `id` = '{$set}' AND `setsale` > '0' LIMIT 1;"))
                    {
                        $good = 1;
                    }
                    else {
                    ...
                    if ($good) {
                        //db_query("UPDATE `shop` SET `count`=`count`-{$_POST['count']} WHERE `id` = '{$set}' LIMIT 1;");
                        ...
                        db_query("UPDATE `users` set `money` = `money`- '".$dress['setsale']."' WHERE id = {$_SESSION['uid']}");
                        db_query("UPDATE `users` set `money` = `money`+ '".$moneyto."' WHERE id = {$userfrom['id']}");
                        db_query("INSERT INTO `delo` (`id` , `author` ,`pers`, `text`, `type`, `date`) VALUES ('','0','{$_SESSION['uid']}','\"".$user['login']."\" купил товар: \"".$dress['name']."\" ".($dress["koll"]?"($dress[koll] шт.)":"")." id:(cap".$dress['id'].") [".$dress['duration']."/".$dress['maxdur']."] от \"".$userfrom['login']."\" за ".$dress['setsale']." кр. в комиссионке кр ($user[money]/$user[ekr])',5,'".time()."');");
                        db_query("INSERT INTO `delo` (`id` , `author` ,`pers`, `text`, `type`, `date`) VALUES ('','0','{$userfrom['id']}','\"".$user['login']."\" купил товар: \"".$dress['name']."\" ".($dress["koll"]?"($dress[koll] шт.)":"")." id:(cap".$dress['id'].") [".$dress['duration']."/".$dress['maxdur']."] от \"".$userfrom['login']."\" за ".$dress['setsale']." кр. в комиссионке ',5,'".time()."');");
                        db_query("INSERT INTO `delo` (`id` , `author` ,`pers`, `text`, `type`, `date`) VALUES ('','0','{$_SESSION['uid']}','\"".$user['login']."\" купил товар: \"".$dress['name']."\" ".($dress["koll"]?"($dress[koll] шт.)":"")." id:(cap".$dress['id'].") [".$dress['duration']."/".$dress['maxdur']."] за ".$dress['setsale']." кр. в комиссионке ',1,'".time()."');");
                        db_query("INSERT INTO `delo` (`id` , `author` ,`pers`, `text`, `type`, `date`) VALUES ('','0','{$userfrom['id']}','\"".$userfrom['login']."\" получил ".$moneyto." кр. за продажу товара: \"".$dress['name']."\" ".($dress["koll"]?"($dress[koll] шт.)":"")." id:(cap".$dress['id'].") [".$dress['duration']."/".$dress['maxdur']."] через комиссионку ',1,'".time()."');");
                        $user['money']=$user['money']-$dress['setsale'];
                        if ($dress['owner']) {
                          $us = mysqli_fetch_array(db_query("select `id` from `online` WHERE `date` >= ".(time()-60)." AND `id` = '{$userfrom['id']}' LIMIT 1;"));
                          if($us[0]){
                              ...
                              // если в офе
                              db_query("INSERT INTO `telegraph` (`owner`,`date`,`text`) values ('".$userfrom['id']."','','".'<font color=red>Внимание!</font> Успешно продан предмет "'.$dress['name'].'" за '.$dress['setsale'].' кр.  Комиссия составила '.$komiss.' кр. Вам перечислено от комиссионного магазина '.$moneyto.' кр. '."');");
                          }
                        }
                    }
                }
    ...
    if ($_GET['unsale'] && $_GET['kredit'] && $_GET['id']) {
        if ((is_numeric($_GET['kredit']) && $_GET['kredit']>0) && (is_numeric($_GET['id']) &&  $_GET['id']>0)) {
            $dress = mysqli_fetch_array(db_query("SELECT * FROM `inventory` WHERE `dressed`=0 AND `id` = '{$_GET['id']}' AND `owner` = '{$_SESSION['uid']}' AND `setsale` > 0 LIMIT 1;"));
            $prc=itemprice($dress);
            if ($_GET['kredit']<$prc["minprice"]*1.1) {
              echo "<font color=red><b>Минимальная цена продажи ".($prc["minprice"]*1.1)." кр</b></font>";
            } elseif ($_GET['kredit']>$prc["maxprice"]*50) {
              echo "<font color=red><b>Максимальная цена продажи ".($prc["maxprice"]*50)." кр</b></font>";              
                ...
                if($user['money'] >= 0.1) {
                    db_query("UPDATE `inventory` SET `setsale` = '".$_GET['kredit']."' WHERE `id` = '{$_GET['id']}' AND `owner` = '{$_SESSION['uid']}' LIMIT 1;");

                    db_query("UPDATE `users` set `money` = `money`- '0.1' WHERE id = {$_SESSION['uid']}");
                    db_query("INSERT INTO `delo` (`id` , `author` ,`pers`, `text`, `type`, `date`) VALUES ('','0','{$_SESSION['uid']}','\"".$user['login']."\" заплатил 0.1 кр за смену цены на предмет \"".$dress['name']."\" ".($dress["koll"]?"($dress[koll] шт.)":"")." id:(cap".$dress['id'].") [".$dress['duration']."/".$dress['maxdur']."] в комиссионке кр ($user[money]/$user[ekr]) ',1,'".time()."');");
...
<?
if ($_REQUEST['max']) {
    $data = db_query("SELECT * FROM `inventory` WHERE  `dressed`=0 AND `name` LIKE '%".db_escape_string($_REQUEST['max'])."%' AND `setsale` > 0 ORDER by `setsale` ASC");
    if ($user['id'] == 7) {
        echo urldecode($_REQUEST['max']);
        //die("SELECT * FROM `inventory` WHERE  `dressed`=0 AND `name` LIKE '%".db_escape_string($_REQUEST['max'])."%' AND `setsale` > 0 ORDER by `setsale` ASC");
...
elseif ($_REQUEST['sale']) {
    ...
    $data = db_query("SELECT * FROM `inventory` WHERE `owner` = '{$_SESSION['uid']}' $cond ORDER by `update` DESC; ");
    while($row = mysqli_fetch_array($data)) {
        $row['count'] = 1;
        if ($i==0) { $i = 1; $color = '#C7C7C7';} else { $i = 0; $color = '#D5D5D5'; }
        echo "<TR bgcolor={$color}><TD align=center style='width:150px'><IMG SRC=\"i/sh/{$row['img']}\" BORDER=0>";
    ...
    $data = db_query("SELECT * FROM `inventory` WHERE `setsale` > 0 AND `owner` = '{$_SESSION['uid']}' AND `dressed` = 0 ORDER by `update` DESC; ");
    while($row = mysqli_fetch_array($data)) {
        $row['count'] = 1;
        if ($i==0) { $i = 1; $color = '#C7C7C7';} else { $i = 0; $color = '#D5D5D5'; }
        echo "<TR bgcolor={$color}><TD align=center style='width:150px'><IMG SRC=\"i/sh/{$row['img']}\" BORDER=0>";
...
{
    $data = db_query("SELECT DISTINCT `img`, `name`, `nalign`,`massa` FROM `inventory` WHERE  `dressed`=0 AND `setsale` > 0 AND `present` = '' ".($_GET["otdel"]==190?" and type=190 ":"AND `otdel` = '".$_GET['otdel']."'")." GROUP BY `img` ORDER by `cost` ASC");
    //echo db_error();
    while($row = mysqli_fetch_array($data)) {
        ...
        $item = mysqli_fetch_array(db_query("SELECT count(`id`), min(duration), min(maxdur), max(duration), max(maxdur), min(setsale), max(setsale) FROM `inventory` WHERE  `dressed`=0 AND `setsale` > 0 AND `present` = '' AND name LIKE '".$item_name."%';"));
        //$row['count']=1;
        if ($i==0) { $i = 1; $color = '#C7C7C7';} else { $i = 0; $color = '#D5D5D5'; }
        ?>
        <TR bgcolor=<?=$color?>>
