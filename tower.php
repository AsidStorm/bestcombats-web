  ...
  $minid=757;
    ...
    class predbannik_bs {
        ...
        function predbannik_bs () {
            ...
            $this->mysql = $mysql;
            $this->userid = $user;
            $this->turnirstart = mysqli_fetch_array(db_query("SELECT `value` FROM `variables` WHERE `var` = 'startbs' LIMIT 1;"));
            $this->turnirstart = $this->turnirstart[0];
        }

        // проверить идет ли турнир?
        function get_turnir () {
            $data = mysqli_fetch_array(db_query("SELECT * FROM `deztow_turnir` WHERE `active` = TRUE"));
        ...
        function start_turnir () {
            ...
            db_query("LOCK TABLES `shop` WRITE, `deztow_items` WRITE, `deztow_realchars` WRITE, `deztow_charstams` WRITE, `deztow_eff` WRITE, `deztow_gamers_inv` WRITE,`effects` WRITE, `deztow_turnir` WRITE, `deztow_stavka` WRITE, `users` WRITE, `inventory` WRITE, `online` WRITE;");
            $minroom = 501;
            $maxroom = 560;
            // вычисляем кто прошел в турнир
            $data = db_query("SELECT dt.owner FROM `deztow_stavka` as dt, `online` as o WHERE o.id = dt.owner AND room = 31 AND o.`date` >= '".(time()-300)."' ORDER by `kredit` DESC, dt.`time` ASC  LIMIT 40;");
            ...
            while($row=mysqli_fetch_array($data)) {
                ...
                $shmot = db_query("SELECT * FROM `inventory` WHERE `owner` = '".$row[0]."';");// бекапим весь шмот
                db_query("UPDATE `inventory` SET `owner` = 0 WHERE `owner` = '".$row[0]."';");
                while($sh = mysqli_fetch_array($shmot)) {
                    db_query("INSERT `deztow_gamers_inv` (`id_item`,`owner`) values ('".$sh[0]."','".$row[0]."');");
                }
                // effects
                $effs = db_query("SELECT * FROM `effects` WHERE `owner` = '".$row[0]."';"); // бекапим ефекты
                while($eff = mysqli_fetch_array($effs)) {
                    deltravma($eff['id']);
                    db_query("INSERT `deztow_eff` (`type`, `name`, `time`, `sila`, `lovk`, `inta`, `vinos`, `owner`)
                    values ('".$eff['type']."','".$eff['name']."','".$eff['time']."',
                    '".$eff['sila']."','".$eff['lovk']."','".$eff['inta']."','".$eff['vinos']."','".$eff['owner']."');");
                }
                db_query("DELETE FROM `effects` WHERE `owner` = '".$row[0]."';");
                // stats
                $tec = mysqli_fetch_array(db_query("SELECT * FROM `deztow_charstams` WHERE `owner` = '{$row[0]}' AND `def`='1';"));
                if($tec[0] && $row[0] != 233) {
                    // умелки
                    $u = mysqli_fetch_array(db_query("SELECT * FROM `users` WHERE `id` = '{$row[0]}' LIMIT 1;"));
                    ...
                    db_query("INSERT `deztow_realchars` (`owner`,`name`,`sila`,`lovk`,`inta`,`vinos`,`intel`,`mudra`,`stats`,`nextup`,`level`,`master`) values
                    ...
                    db_query("UPDATE `users` SET `sila`='".$tec['sila']."', `lovk`='".$tec['lovk']."',`inta`='".$tec['inta']."',`vinos`='".$tec['vinos']."',`intel`='".$tec['intel']."',`mudra`='".$tec['mudra']."',`stats`='".$stats."',
                ...
                db_query("UPDATE `users` SET `in_tower` = 1, `room` = '".$rum."' WHERE `id` = '".$row[0]."';");
                db_query("UPDATE `online` SET `room` = '".$rum."' WHERE `id` = '".$row[0]."' LIMIT 1 ;");
                // в список участников
                $i++;
                if($i>1) { $lors .= ", "; }
                $lors .= nick3($row[0]);
            ...
            // arch================
                undressall(233);
                db_query("DELETE FROM `inventory` WHERE `owner` = '233';");
                $rum = rand($minroom,$maxroom);
                db_query("UPDATE `users` SET `in_tower` = 1, `chattime` = '999999999999', `room` = '".$rum."' WHERE `id` = '233';");
                db_query("UPDATE `online` SET `room` = '".$rum."' WHERE `id` = '233' LIMIT 1 ;");
                $i++;
                $lors .= ", ".nick3(233);
            //=====================

            // разбрасываем шмот по комнатам
            db_query("TRUNCATE TABLE `deztow_items`;");
            ...
            while($sh = array_shift($shmots)) {
                $shopid = mysqli_fetch_array(db_query("SELECT * FROM `shop` WHERE `id` = '".$sh."' LIMIT 1;"));
            ...
            db_query("INSERT `deztow_turnir` (`type`,`winner`,`coin`,`start_time`,`log`,`endtime`,`active`) values ('".rand(1,7)."','','".$stavka[0]."','".time()."','".$log."','0','1');");
        ...
        // проверить ставку
        function get_stavka () {
            $data = mysqli_fetch_array(db_query("SELECT `kredit` FROM `deztow_stavka` WHERE `owner` = '".$this->userid['id']."' LIMIT 1;"));
            return $data[0];
        }
        ...
        function set_stavka ($kredit) {
          if (FREEGAME) {
            db_query("INSERT `deztow_stavka` (`owner`,`kredit`,`time`) values ('".$this->userid['id']."','".FREEGAME."','".time()."' ); ");
          } else {
            if($kredit >= 3 && $this->userid['level'] > 3 && $this->userid['money'] >= $kredit) {
              db_query("INSERT `deztow_stavka` (`owner`,`kredit`,`time`) values ('".$this->userid['id']."','".(float)$kredit."','".time()."' ); ");
              db_query("UPDATE `users` set `money` = `money`- '".$kredit."' WHERE id = {$_SESSION['uid']}");
        ...
        function up_stavka ($kredit) {
          if($kredit >= 1 && $this->userid['level'] > 3 && $this->userid['money'] >= $kredit) {
            db_query("UPDATE `deztow_stavka` SET `kredit` = `kredit`+'".(float)$kredit."' WHERE `owner` = '".$this->userid['id']."';");
            db_query("UPDATE `users` set `money` = `money`- '".$kredit."' WHERE id = {$_SESSION['uid']}");
            //echo db_error();
          }
        }
        ...
        function get_fond () {
            $data = mysqli_fetch_array(db_query("SELECT SUM(`kredit`)*0.7, count(`kredit`) FROM `deztow_stavka`;"));
...
?>
    ...
    <?
        ...
        if (!FREEGAME) echo "Чем выше ваша ставка, тем больше шансов принять участие в турнире. Подробнее о башне смерти читайте в разделе \"Подсказка\".";
            ...
            //$ls = mysqli_fetch_array(db_query("select count(`id`) from `users` WHERE `in_tower` = 1;"));
            ...
            $i=0;
            while($user1=mysqli_fetch_array($lss)) {
                $i++;
                if($i>1) { $lors .= ", "; }
        ...
        ?>
            ...
            <?=$tr['log']?><BR>
            Всего живых участников на данный момент: <B><?=mysqli_num_rows($lss)?></B> (<?=$lors?>)
            <?
              if (mysqli_num_rows($lss)==0 && time()-$tr["start_time"]>600 && !$tr["endtime"]) {
                $dtt=mqfa1("select value from variables where var='deztowtype'");
...
<?
  $data = mysqli_fetch_array(db_query("SELECT * FROM `deztow_turnir` where active=0 and id>$minid ORDER by `coin` DESC LIMIT 1;"));
  if ($data) { ?>
...
<?
  ...
  $data = mysqli_fetch_array(db_query("SELECT * FROM `deztow_turnir` where active=0 and id>$minid ORDER by (`endtime`-`start_time`) DESC LIMIT 1;"));
  if ($data) { ?>
