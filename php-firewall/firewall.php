...
define('PHP_FIREWALL_PROTECTION_REQUEST_METHOD', false );
...
//$ds=header('Location: http://fsb.ru');
//exit;
/** LANGUAGE */
if ( PHP_FIREWALL_LANGUAGE === 'french' ) {
	define('_PHPF_PROTECTION_DEDIBOX', 'Protection contre les serveurs DEDIBOX active, cette IP range n\'est pas autorisйe !');
	define('_PHPF_PROTECTION_DEDIBOX_IP', 'Protection contre les serveurs DEDIBOX active, cette IP range n\'est pas autorisйe !');

	define('_PHPF_PROTECTION_DIGICUBE', 'Protection contre les serveurs DIGICUBE active, this IP range is not allowed !');
	define('_PHPF_PROTECTION_DIGICUBE_IP', 'Protection contre les serveurs DIGICUBE active, cette IP n\'est pas autorisйe !');

	define('_PHPF_PROTECTION_KIMSUFI', 'Protection contre les serveurs KIMSUFI active, cette IP range n\'est pas autorisйe !');
	define('_PHPF_PROTECTION_OVH', 'Protection contre les serveurs OVH active, cette IP range n\'est pas autorisйe !');

	define('_PHPF_PROTECTION_BOTS', 'Attaque Bot dйtectйe ! stop it ...');
	define('_PHPF_PROTECTION_CLICK', 'Click attaque dйtectйe ! stop it .....');
	...
	define('_PHPF_PROTECTION_REQUEST', 'Mйthode de requкte interdite ! Stop it ...');
	define('_PHPF_PROTECTION_SANTY', 'Attaque Santy detectйe ! Stop it ...');
	define('_PHPF_PROTECTION_SPAM', 'Protection SPAM IPs active, cette IP range n\'est pas autorisйe !');
	define('_PHPF_PROTECTION_SPAM_IP', 'Protection SPAM IPs active, cette IP range n\'est pas autorisйe !');
	define('_PHPF_PROTECTION_UNION', '');
	define('_PHPF_PROTECTION_URL', 'Protection url active, string non autorisйe !');
	define('_PHPF_PROTECTION_XSS', 'Attaque XSS dйtectйe ! stop it ...');
} else {
	...
	define('_PHPF_PROTECTION_UNION', 'Здесь скулей нету!');
...
if ( PHP_FIREWALL_ACTIVATION === true ) {
	...
	FUNCTION PHP_FIREWALL_get_ip() {
		if ( PHP_FIREWALL_get_env('REMOTE_ADDR') ) {
			return PHP_FIREWALL_get_env('REMOTE_ADDR');
		} elseif ( PHP_FIREWALL_get_env('REMOTE_ADDR') ) {
			return PHP_FIREWALL_get_env('REMOTE_ADDR');
		} else {
			return PHP_FIREWALL_get_env('REMOTE_ADDR');
		}
	...
	if ( PHP_FIREWALL_PROTECTION_SERVER_OVH === false ) {
	...
	if ( PHP_FIREWALL_PROTECTION_SERVER_OVH_BY_IP === false ) {
	...
	if ( PHP_FIREWALL_PROTECTION_UNION_SQL === true ) {
		$stop = 0;
		$ct_rules = array( '*/from/*', '*/insert/*', '+into+', '%20into%20', '*/into/*', ' into ', 'into', '*/limit/*', 'not123exists*', '*/radminsuper/*', '*/select/*', '+select+', '%20select%20', ' select ',  '+union+', '%20union%20', '*/union/*', ' union ', '+delete+', '%20delete%20', '*/delete/*', '+delete+', 'union', 'selet', '+from+', '*/update/*', '*/where/*' );
		$check    = str_replace($ct_rules, '*', PHP_FIREWALL_GET_QUERY_STRING );
