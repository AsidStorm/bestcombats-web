...
class prieminfo{
  ...
  function prieminfo($s,$priem) { # либо по id ($s) либо по названию $priem
    ...
    foreach ($strokes[$priem] as $k=>$v) {
	  ...
	  if ($k=="need_int") $k="needint";
	  if ($k=="need_lovk") $k="needlovk";
    ...
    $this->needlovk=$res['need_lovk'];
    $this->needint=$res['need_int'];
  }
  function check_hars($n) {
global $user; # проверка. n=0: все хар-ки n=1: только жизнь и мана
    if($n==0) {

      if(($this->minlevel<=$user['level']) && ($this->intel<=$user['intel']) && ($this->needsil<=$user['sila']) && ($this->needint<=$user['inta']) && ($this->needlovk<=$user['lovk']) && ($this->needvyn<=$user['vinos']) && ($this->m_magic1<=$user['mfire']) && ($this->m_magic2<=$user['mwater']) && ($this->m_magic3<=$user['mair']) && ($this->m_magic4<=$user['mearth']) && ($this->m_magic5<=$user['mlight']) && ($this->m_magic6<=$user['mgray']) && ($this->m_magic7<=$user['mdark']) ) {
        return true;
      }else{return false;}
    }elseif($n==1){
  ...
  $user['lovk']>=$this->needlovk &&
  $user['int']>=$this->needint &&
...
echo "style=\"-moz-opacity:.70; opacity:.70;filter:gray(), Alpha(Opacity='70');\"";
        ...
        OR $prinfo->m_magic3 OR $prinfo->m_magic4 OR $prinfo->m_magic6) {
            ...
            if ($prinfo->needlovk) {
                echo ($prinfo->needlovk>$user['lovk']?"<font color=red>":"").
                "Ловкость: ".$prinfo->needlovk."<BR>".
                ($prinfo->needlovk>$user['lovk']?"</font>":"");
            }
            if ($prinfo->needint) {
                echo ($prinfo->needint>$user['int']?"<font color=red>":"").
                "Интуиция: ".$prinfo->needint."<BR>".
                ($prinfo->needint>$user['int']?"</font>":"");
            }
...
for ($i=201;$i<=205;$i++) {
    ...
    if ($puton[$i]) {
        ...
        echo"<IMG style=\"\" width=40 height=25 src='/i/misc/icons/clear.gif'>";
    }
    echo "</TD>";
}unset($i);
...
if ($row["mf"]) {
  ...
  $i=0;
  $mfnames=array("mfuvorot"=>"мф. увёртливости", "mfauvorot"=>"мф. против увёртливости");
  foreach ($tmp as $k=>$v) {
    $i++;
    if ($i>1) echo ", ";
...
if ($user["mudra"]>=25) {
  ...
  <li>Понижение защиты от магии: +3%</li>";
...
if ($znTowerLevel) {
    ...
    if ($znTowerLevel >= 10000) {
        $znRep = 'Посвящённый 3-го круга, ';
        $znRepNext = 'Бесконечно';
    }
} else {
    $znTowerLevel = 0;
}
