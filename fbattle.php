...
define("SNOWBALLSDROP", 0);
define("SNOWBALL",0);
$finishscript="";
$notacticrestrrooms=array(72);

if (!defined("DOCUMENTROOT")) {
define("DOCUMENTROOT","/var/www/bestcombats/data/www/bestcombats.net/");
}
...
function drawtrick($can_use, $img,  $txt, $free_cast, $dsc, $resource, $select_target, $target, $target_login, $magic_type, $name) {
    ...
    if ($can_use) {
        if ($select_target) {
            $ret.="
                <A style='cursor: pointer' onclick=\"tricklogin('</b>Выберите".
...
function geteffectval($val, $len, $maxlen) {
      ...
      foreach ($fbattle->battleunits[$user["id"]]["additdata"]["scrolls"] as $k=>$dress) {
        if ($dress) {
          if ($dress['magic'] && !$fbattle->battleunits[$user["id"]]["additdata"]["scrollused"] && !$dress["wait"]) {
            ...
            $ret.="<a  onclick=\"";
            if($magic['targeted']==1) {
              $ret.="okno('Введите название предмета', 'fbattle.php?use=$dress[id]', 'target'); ";
            } else
            if($magic['targeted']==2) {
              $ret.="findlogin('Введите имя персонажа', 'fbattle.php?use=$dress[id]', 'target', ''); ";
            } else
            if($magic['targeted']==4) {
              $ret.="note('Запрос', 'fbattle.php?use=$dress[id]', 'target'); ";
            } else {
              $ret.="if(confirm('Использовать сейчас?')) { window.location='fbattle.php?use=".$dress['id']."';}";
            }
            $ret.="\" href='#'>";
          }
  ...
  // ставим блокировку на таблицу
    ...
    mq("LOCK TABLES `podzem_drop` write, fielditems write, bots WRITE, `puton` WRITE, `userdata` WRITE, `priem` WRITE, `deztow_realchars` write, `shop` WRITE, `person_on` WRITE, `podzem3` WRITE, `canal_bot` WRITE, `labirint` WRITE, `battle` WRITE, `logs` WRITE, `users` WRITE, `inventory` WRITE, `magic` WRITE, `effects` WRITE, `clans` WRITE, `online` WRITE, `telegraph` WRITE, `users` WRITE, `quests` WRITE, `battleeffects` WRITE, `items` WRITE, `podzem2` WRITE, `battleunits` WRITE, `berezka` WRITE, `qtimes` WRITE, `smallitems` WRITE, `podzem_zad_login` write, `caveparties` write, `caveitems` write, `cavebots` write, errorstats write, caves write, userstrokes write, variables write, droplog write, fieldparties write, fields write, obshagaeffects write, invisbattles write, clanstorage write, inventory write, fieldlogs write, effectimgs write, includemagicuses write;");
...
class fbattle {
  ...
  function makemagicdamage($user, $enemy, $magic) {
    ...
    $dam=rand($magic["from"], $magic["to"])*(1-mftoabs($this->battleunits[$enemy]["mfdmag"]+$this->battleunits[$enemy]["mfd".$magic["prof"]]))*10;
      ...
      if(($t2life == 0 OR $t1life == 0) && ($charge[0] == 3 || $charge[0] == 9)) {
        ...
        if(in_array($ss[0],$this->t1)) {
          ...
          if (in_array($user["room"], $caverooms)) {
            ...
            mq("update cavebots set battle=0 where battle='".$this->battle_data["id"]."'");
          } elseif (in_array($user["room"], $caverooms)) {
            ...
            if($dd=mysqli_fetch_array($sd)) {
              ...
              mq("UPDATE `labirint` SET `location`='".$podzemdata[$this->user["room"]]["location"]."',`vector`='".$podzemdata[$this->user["room"]]["vector"]."',`dead`=dead+1,`t`='".$podzemdata[$this->user["room"]]["t"]."',`l`='".$podzemdata[$this->user["room"]]["l"]."',`boi`='0',`di`='1',`name`='$podzname' WHERE `glav_id`=".$dd['glav_id']." and name='$dd[name]' and boi='$dd[boi]'");
            }
          }
          ///////////////////////////////////
  ...
  function end_gora() {
      // я - царь горы
          if ($this->get_timeout()) {
              ...
              //header("Location: fbattle.php?fd=all&batl=".$this->user['battle']);
          }
  }
...
if($user['room']=='403' || $user['room']=='903' || $user['room']=='302'){
                $result = (($baseexp[$def['level']])*($damage/$def['maxhp'])*$expmf*$mfit*$mfbot*$mfbot2)/0.4; 
} else {
...
function razmen_log($type,$kuda,$chem,$uron,$kto,$c1,$pokomy,$c2,$hp,$maxhp, $block, $prof=0) {
  ...
  if ($kto>_BOTSEPARATOR_) {
    ...
    if ($this->battleunits[$kto]["prototype"]==21302) $textchem=array("лапой", "когтями");
    if ($this->battleunits[$kto]["prototype"]==21303) $textchem=array("лапой", "когтями");
  }
  // куда били
  $udars = array(
    ...
    if ($enable) {
      ...
      if ($ok) {
        ...
        if (@$strokes[$priem]->refresh) {
          //if (!NOREFRESH) header ("Location:fbattle.php?fd=".$fbattle->enemy);
          $fbattle->needupdate=1;
          //die;
        }
        if(@$_POST['enemy']>0 && $emptyhit) {
          ...
          //if (!NOREFRESH) header ("Location:fbattle.php");
          //die;
        }
        $fbattle->write_log();
...
if ($fbattle->needupdate) {
  ...
  if (@$fbattle->needrefresh || $pershp<=0) {
    ...
    header ("Location:fbattle.php?".time()."&if=".@$_GET["if"]);
    $fbattle->updateaddit();
    die;
  }
} else {
...
<div id="screen">
<form name="emptyhit" action="fbattle.php" method="post" style="margin:0px;padding:0px;visibility:hidden;display:none">
...
<td align=center>
&nbsp;
<INPUT TYPE=button value="Обновить" onClick="this.disabled = true; document.location.href='fbattle.php';">
</td>
            ...
            elseif($user['hp'] <= 0 && $fbattle->battle) {
                if ($fbattle->battle_data["quest"]==2) {
                  ...
                  if ($timeloser) echo "<center><BR>Персонаж ".mqfa1("select login from users where id='$timeloser'")." долго не делает свой ход, вы можете исключить его из боя.<BR>
                    <INPUT TYPE=button value=\"Признать его проигравшим\" onclick=\"document.location.href='fbattle.php?killplayer=$timeloser';\"><BR>
