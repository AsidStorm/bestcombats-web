...
<tr>
	<td valign=top>
		<center>
			<h3><A HREF="javascript:top.AddToPrivate('pal', top.CtrlPress)" target=refreshed><img src="i/lock.gif" width=20 height=15></A> Орден Света</h3>		
		<table>
			<tr>
			<td>
				<?
					$data=db_query("SELECT `id`, `login`, `status`, `level`, `room`, `align`, (select `id` from `online` WHERE `date` >= ".(time()-60)." AND `id` = users.`id`) as `online` FROM `users` WHERE `align` > 1 and `align` < 2 order by  align desc, login asc ;");
