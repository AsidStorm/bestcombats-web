<?
  ...
  function updmap() {
   ...
   if ($user["sex"]==1){
     $row="i/shadow/1/$user[shadow]";
         }
      elseif ($user["sex"]==0){ 
      $row="i/shadow/0/$user[shadow]";
    }
  function drawuser($user, $x, $y) {
    global $imgdata, $udata, $row;
    $bn=1;
    $ret.="<img title=\"".$udata[$user]["login"]."\" ".($y==1 && $x==3?"onclick=\"attackmenu(event);\"":"")." width=\"".$imgdata[$x][$y]["wd"]."\" height=\"".$imgdata[$x][$y]["ht"]."\" src=/$row style=\"position:absolute;left:".$imgdata[$x][$y]["x"][$bn]."px;top:".$imgdata[$x][$y]["y"]."px;".($x==3 && $y==1?"cursor:pointer;":"").($x==3?"z-index:".(99-($y*5)).";":"")."\">";
    return $ret;
  }
