...
if($_GET['act']=="atk"){
$ass = db_query("SELECT glav_id,glava,name FROM labirint WHERE user_id=".$user['id']."");
$lab = mysqli_fetch_array($ass);
$glav_id = $lab["glav_id"];
$asx = db_query("SELECT login FROM labirint WHERE glav_id='$glav_id' and boi='".$_GET['n']."'");
if(!$lax = mysqli_fetch_array($asx)){
$f = db_query("SELECT `n".$_GET['n']."` FROM podzem3 WHERE glava='".$lab["glava"]."' and name='".$lab["name"]."'");
$rt = mysqli_fetch_array($f);
...
$jert = mysqli_fetch_array(db_query("SELECT * FROM `users` WHERE `login` = '".$lax["login"]."' LIMIT 1;"));
if($jert['id']!=$user['id']) {
...
db_query("UPDATE `labirint` SET `boi`='".$_GET['n']."',`di`='0' WHERE `user_id`='".$user['id']."'");
