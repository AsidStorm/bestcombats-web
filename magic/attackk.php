...
if ($cant && $user['id'] != 7) {
  echo $cant;
} elseif ($cant2 && $user['id'] != 7) {
  echo $cant2;
} elseif ($user["align"]==4) {
    echo "Хаосникам запрещены кулачные нападения.";
} else
//if (mt_rand(1,100) < $int)
{
  takefromzayavka($us["id"]);
            ...
            if($jert['id']!=$user['id']) {
                if ($user['invis']==1) {
                addch("<img src=i/magic/attackk.gif> <B>невидимка</B>, применив магию кулачного нападения, внезапно ".$action." на &quot;{$_POST['target']}&quot;");
                ...
                addch("<img src=i/magic/attackk.gif> <B>{$user['login']}</B>, применив магию кулачного нападения, внезапно ".$action." на &quot;{$_POST['target']}&quot;");
                ...
                else
                {
                    ...
                    $sv = array(1,1,1);
                    //$tou = array_rand($sv,1);
                    mq("INSERT INTO `battle` (`id`,`coment`,`teams`,`timeout`,`type`,`status`,`t1`,`t2`,`to1`,`to2`,`blood`, date)
                    VALUES (NULL,'Кулачное нападение','".serialize($teams)."','".$sv[rand(0,2)]."','4','0','".$user['id']."','".$jert['id']."','".time()."','".time()."','0', '".date("Y-m-d H:i")."')");
                    $id = db_insert_id();
        ...
        //echo "<font color=red>Свиток рассыпался в ваших руках...</font>";
//  }
?>
