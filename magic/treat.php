...
$travma2 = db_query("SELECT id FROM `effects` WHERE `owner` = '".$us['id']."' AND (`type`='11' OR `type`='12' OR `type`='13') and id not in (select effect from treats where user='$user[id]');");
//$owntravma2=mysqli_fetch_array($travma);
if ($user['battle'] > 0) {
  echo "Не в бою...";}
elseif ($us['battle'] > 0) {
  echo "Персонаж в бою...";} 
elseif (mysqli_num_rows($travma2)==0) {
