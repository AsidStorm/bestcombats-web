...
echo'Не достаточно духа';
		...
		$magic = mysqli_fetch_array(db_query("SELECT `chanse` FROM `magic` WHERE `id` = '12' ;"));
		if ($user['intel'] >= 6) {
		...
		elseif ($us['battle'] && !in_array($us['id'],$fbattle->team_mine)) { echo "Нельзя лечить противников!"; }
			...
			if (rand(1,100) < $int) {	
				...
				if ($user['battle'] > 0) {
					//db_query('UPDATE `logs` SET `log` = CONCAT(`log`,\'<span class=date>'.date("H:i").'</span> '.nick5($user['id'],$fbattle->my_class).' использовал заклятие восстановления энергии '.(($us['id']!=$user['id'])?"на ".nick5($us['id'],$fbattle->my_class):"").' и восстановил уровень жизни <B>+1500</B> ['.($hp).'/'.$us['maxhp'].']<BR>\') WHERE `id` = '.$us['battle'].'');
					$fbattle->add_log('<span class=date>'.date("H:i").'</span> '.nick5($user['id'],$fbattle->my_class).' использовал'.$action.' заклятие восстановления энергии '.(($us['id']!=$user['id'])?"на ".nick5($us['id'],$fbattle->my_class):"").' и восстановил'.$action.' уровень жизни <B>+1500</B> ['.($hp).'/'.$us['maxhp'].']<BR>');
					//$fbattle->add_log('');
				...
				db_query("UPDATE `users` SET `hp` = ".$hp.",`s_duh` = s_duh-1000 WHERE `id` = ".$us['id'].";");
                                global $updatebattleunit;
                                $updatebattleunit=$us['id'];
				echo "Вы восстановили 1500 НР персонажу ".$us['login']."!";
				$bet=1;
			} else {
