...
if ($user['battle'] == 0) {
	echo "Это боевая магия...";
			...
			db_query("UPDATE users SET `battle` =0, `hp` = 0, `maxhp` = 0, `mana` = 0, `maxmana` = 0,`hp2` = 0,`hp3` = 0,`hit` = 0,`s_duh` = 0,`krit` = 0,`counter` = 0,`block2` = 0,`parry` = 0 WHERE `id` = '{$user['id']}';");
			db_query("DELETE FROM `person_on` WHERE `id_person`='".$user['id']."'");
