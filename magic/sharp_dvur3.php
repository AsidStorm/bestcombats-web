<?php
// magic идентификацыя
$magic = mysql_fetch_array(mysql_query("SELECT `chanse` FROM `magic` WHERE `id` = '93' ;"));  
...
if ($user['battle'] > 0) {
	echo "Не в бою...";
		...
		$svitok = mysql_fetch_array(mysql_query("SELECT * FROM `inventory` WHERE `name` = 'Заточка на +3: двуручное оружие' AND `owner` = '{$user['id']}' LIMIT 1;"));
