...
// magic идентификацыя
$magic = mysqli_fetch_array(db_query("SELECT `chanse` FROM `magic` WHERE `id` = '106' ;"));
...
if ($user['battle'] > 0) {
  echo "Не в бою...";
                ...
                $svitok = mysqli_fetch_array(db_query("SELECT * FROM `inventory` WHERE `name` = 'Заточка на +1: оружие' AND `owner` = '{$user['id']}' LIMIT 1;"));

                if ($dress && $svitok) {
                  $weptype=$weptypes[$dress["otdel"]];
                  if ($weptype) {
                    if (db_query("UPDATE `inventory` SET `sharped` = 1, `name` = CONCAT(`name`,' +1'), `minu` = `minu`+1, `maxu`=`maxu`+1, `n$weptype` = `n$weptype`+1, `cost` = `cost`+6, `nvinos` = `nvinos`+0 WHERE `id` = {$dress['id']} LIMIT 1;")) {
