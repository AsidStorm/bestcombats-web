<?php
// magic идентификацыя
$magic = mysqli_fetch_array(db_query("SELECT `chanse` FROM `magic` WHERE `id` = '114' ;"));
...
if ($user['battle'] > 0) {
	echo "Не в бою...";
		...
		$svitok = mysqli_fetch_array(db_query("SELECT * FROM `inventory` WHERE `name` = 'Заточка на +9: оружие' AND `owner` = '{$user['id']}' LIMIT 1;"));
