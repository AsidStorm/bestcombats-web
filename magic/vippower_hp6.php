...
$magic = mysqli_fetch_array(db_query("SELECT `chanse` FROM `magic` WHERE `id` = '204' ;"));
$effect = mysqli_fetch_array(db_query("SELECT `time` FROM `effects` WHERE `owner` = '{$us['id']}' and `type` = '901' LIMIT 1;"));
}
else
{ 
$effect = mysqli_fetch_array(db_query("SELECT `time`,`id` FROM `effects` WHERE `owner` = '{$_SESSION['uid']}' and `type` = '901' LIMIT 1;"));
}
...
if($_POST['target']!=""){
	  ...
	  $addhp=$user['vinos']*6;
      if ($effect['time']){db_query("UPDATE `effects` SET `hp`='".$addhp."',`time`=".(time()+7200)." WHERE `owner` = '{$user['id']}' and `type` = '901' LIMIT 1;");}
	  else{db_query("INSERT INTO `effects` (`owner`,`hp`,`maxhp`,`name`,`time`,`type`) values ('".$user['id']."','".$addhp."','".$addhp."','Жажда Жизни +6',".(time()+7200).",901);");
