...
$magic = mysql_fetch_array(mysql_query("SELECT `chanse` FROM `magic` WHERE `id` = '64' ;"));  
...
elseif (rand(1,100) < $int) {    
      ...
      mysql_query("INSERT INTO `effects` (`owner`,`name`,`time`,`type`,`stihiya`) values ('".$user['id']."','Астрал стихий (земля)',".(time()+7200).",'400','zemlya');");    
      echo "<font color=red><b>На персонажа \"{$_POST['target']}\" наложено заклятие \"Астрал стихий (земля)\" </b></font>";      
      $bet=1;
