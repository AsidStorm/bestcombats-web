  ...
  $magic = mysqli_fetch_array(db_query("SELECT `chanse` FROM `magic` WHERE `id` = '12' ;"));
        ...
        elseif ($us['battle'] && $us["hp"]<=0) { echo "Нельзя лечить мёртвых!"; }
            ...
            if (rand(1,100) < $int) {
                ...
                if ($user['battle'] > 0) {
                    //db_query('UPDATE `logs` SET `log` = CONCAT(`log`,\'<span class=date>'.date("H:i").'</span> '.nick5($user['id'],$fbattle->my_class).' использовал заклятие восстановления энергии '.(($us['id']!=$user['id'])?"на ".nick5($us['id'],$fbattle->my_class):"").' и восстановил уровень жизни <B>+600</B> ['.($hp).'/'.$us['maxhp'].']<BR>\') WHERE `id` = '.$us['battle'].'');
