...
$magic = mysql_fetch_array(mysql_query("SELECT `chanse` FROM `magic` WHERE `id` = '3000' ;"));  
$effect = mysql_fetch_array(mysql_query("SELECT `time` FROM `effects` WHERE `owner` = '{$us['id']}' and `type` = '9999' LIMIT 1;"));
}
else
{ 
$effect = mysql_fetch_array(mysql_query("SELECT `time`,`id` FROM `effects` WHERE `owner` = '{$_SESSION['uid']}' and `type` = '9999' LIMIT 1;"));
}
...
if($_POST['target']!=""){
     addch("<font color=red>внимание! </font><img src=http://img.vr-bk.net/i/magic/spell_godstat_dex.gif>Персонаж &quot;{$user['login']}&quot; наложил заклятие \"Скорость Змеи\" на &quot;{$_POST['target']}&quot;, сроком 2 часа.");
	  ...
	  else{mysql_query("INSERT INTO `effects` (`owner`,`lovk`,`name`,`time`,`type`) values ('".$user['id']."','100','Скорость Змеи',".(time()+7200).",9999);");}
	  mysql_query("UPDATE `users` SET `lovk`=`lovk`+'100' WHERE `login` = '{$_POST['target']}' LIMIT 1;");
	  ...
	  else{mysql_query("INSERT INTO `effects` (`owner`,`lovk`,`name`,`time`,`type`) values ('".$ptarget['id']."','".floor($ptarget['login']+100)."','Скорость Змеи',".(time()+7200).",9999);");}
	  mysql_query("UPDATE `users` SET `lovk`=`lovk`+'".floor($ptarget['login']+100)."' WHERE `id` = '".$ptarget['id']."' LIMIT 1;");
