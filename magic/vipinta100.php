...
$magic = mysqli_fetch_array(db_query("SELECT `chanse` FROM `magic` WHERE `id` = '203' ;"));
$effect = mysqli_fetch_array(db_query("SELECT `time` FROM `effects` WHERE `owner` = '{$us['id']}' and `type` = '99' LIMIT 1;"));
}
else
{ 
$effect = mysqli_fetch_array(db_query("SELECT `time`,`id` FROM `effects` WHERE `owner` = '{$_SESSION['uid']}' and `type` = '99' LIMIT 1;"));
}
...
if($_POST['target']!=""){
     ...
     $user = mysqli_fetch_array(db_query("SELECT `id` FROM `users` WHERE `login` = '{$_POST['target']}' LIMIT 1;"));
	  ...
	  else{db_query("INSERT INTO `effects` (`owner`,`inta`,`name`,`time`,`type`) values ('".$user['id']."','100','Предчувствие',".(time()+7200).",99);");}
	  db_query("UPDATE `users` SET `inta`=`inta`+'100' WHERE `login` = '{$_POST['target']}' LIMIT 1;");
	  ...
	  else{db_query("INSERT INTO `effects` (`owner`,`inta`,`name`,`time`,`type`) values ('".$ptarget['id']."','".floor($ptarget['login']+100)."','Предчувствие',".(time()+7200).",99);");}
	  db_query("UPDATE `users` SET `inta`=`inta`+'".floor($ptarget['login']+100)."' WHERE `id` = '".$ptarget['id']."' LIMIT 1;");
