...
// magic идентификацыя
    //if (rand(1,2)==1) {
        ...
        $tar = mysqli_fetch_array(db_query("SELECT `id`,`align` FROM `users` WHERE `login` = '{$_POST['ldnick']}' LIMIT 1;"));
        $target=$_POST['ldnick'];
        if ($tar['id']) {
            $ok=0;
            ...
            if ($ok == 1) {
                if ($_POST['red']) {
                    ...
                    if (db_query("UPDATE `users` SET `palcom` = '$pal' WHERE `id` = {$tar['id']} LIMIT 1;")) {
                    ...
                    $mess="Сообщение от ".$user['login'].": ".$_POST['ldtext'];
                    if (db_query("INSERT INTO `lichka`(`id`,`pers`,`text`,`date`) VALUES ('','".$tar['id']."','$mess','".time()."');")) {
